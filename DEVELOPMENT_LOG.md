...
- Navbar uses `next/image` logo from `public/logo.webp` and sample-like layout behavior.
...
- Navbar replicated closer to sample layout/classes and logo sizing (`h-24`) from `public/logo.webp`.
...
- Added `src/components/Sections/HeroSection.tsx` from provided sample style, converted to Next.js (`next/link`, `next/image`, public asset `/hero-epoxy.webp`).
